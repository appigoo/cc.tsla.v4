...
with tabs[-1]:
    ...
    # 回測專屬時間設定（與主監控完全獨立）
    _bt_period_opts   = ["1mo","3mo","6mo","1y","2y","5y","ytd","max"]
    _bt_interval_opts = ["1d","1wk","1mo"]
    st.caption(
        f"ℹ️ 主監控目前：**{selected_period}** / **{selected_interval}**　"
        "回測可獨立選擇更長時間範圍以獲得更多樣本。"
    )
    _col_p, _col_i = st.columns(2)
    bt_period   = _col_p.selectbox(
        "回測時間範圍",
        _bt_period_opts,
        index=_bt_period_opts.index("2y"),
        key="bt_period",
        help="建議 1y 以上，樣本越多勝率越可信",
    )
    bt_interval = _col_i.selectbox(
        ...
        index=0,
        key="bt_interval",
        help="1d=日線（最多5年）　1wk=週線　1mo=月線",
    )
    # 預估K線數量
    _est_map = {
        ("1mo","1d"):30,  ("3mo","1d"):63,  ("6mo","1d"):126, ("1y","1d"):252,
        ("2y","1d"):504,  ("5y","1d"):1260, ("ytd","1d"):180, ("max","1d"):5000,
        ("1mo","1wk"):4,  ("3mo","1wk"):13, ("6mo","1wk"):26, ("1y","1wk"):52,
        ("2y","1wk"):104, ("5y","1wk"):260, ("ytd","1wk"):26, ("max","1wk"):1300,
        ("1mo","1mo"):1,  ("3mo","1mo"):3,  ("6mo","1mo"):6,  ("1y","1mo"):12,
        ("2y","1mo"):24,  ("5y","1mo"):60,  ("ytd","1mo"):12, ("max","1mo"):300,
    }
    _est_n = _est_map.get((bt_period, bt_interval), "?")
    _est_icon = ("🟢" if isinstance(_est_n,int) and _est_n>=100
                 else "🟡" if isinstance(_est_n,int) and _est_n>=30
                 else "🔴")
    st.caption(
        f"{_est_icon} 預估約 **{_est_n}** 根K線"
        + ("（建議 ≥ 100 根，樣本越多勝率越可信）" if isinstance(_est_n,int) and _est_n<100
           else "（樣本充足）" if isinstance(_est_n,int) else "")
    )
    ...
    if st.button("🚀 開始回測", type="primary"):
        with st.spinner(f"正在計算 {bt_ticker}（{bt_period} / {bt_interval}）三維勝率，稍候…"):
            try:
                ...
                st.session_state["bt_period_lbl"]  = bt_period
                st.session_state["bt_interval_lbl"]= bt_interval
                st.session_state["bt_total_bars"]  = len(bt_data)
    ...
    if "bt_df_sig" in st.session_state:
        ...
        _bt_period_used  = st.session_state.get("bt_period_lbl",   "?")
        _bt_interval_used= st.session_state.get("bt_interval_lbl", "?")
        _bt_bars_used    = st.session_state.get("bt_total_bars",   "?")
                        ...
                        if _bt_raw is None:
                            ...
                            if _detail.empty:
                                ...
                                _combo_safe = (_sel_row["信號組合"][:40]
                                               ...
                                               .replace("/","_")
                                               .replace(" ","_"))
                                _ticker_safe = st.session_state.get("bt_ticker_lbl","stock")
                                ...
                                _stats_rows = [
                                    ...
                                    ["持倉根數", int(_hold_bars)],
                                    ["回測時間範圍", st.session_state.get("bt_period_lbl","?")],
                                    ["K線間隔", st.session_state.get("bt_interval_lbl","?")],
                                    ["總K線根數", st.session_state.get("bt_total_bars","?")],
                                    [],
                                ]
